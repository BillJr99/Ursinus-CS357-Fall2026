...
| DeepAgents (LangChain) | Long-horizon, open-ended tasks needing planning, subagents, and a scratch filesystem out of the box | Very high (a batteries-included harness built on LangGraph) | Minimal — `create_deep_agent(model, tools)` and you have planning + files + subagents | Low at first, high underneath — it is LangGraph, so you can drop down when needed | `create_deep_agent`, `write_todos`, virtual filesystem, `task` subagent tool | `pip install deepagents` (covered in its own activity) |
