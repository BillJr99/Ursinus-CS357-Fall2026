...
- (x) Successive API calls to the same endpoint with different system prompts, coordinated by a Python orchestrator and a shared blackboard
...
2. *Notebook run-through.* Complete [OpenWebUI_API_Client_With_Upload.ipynb](/files/notebooks/OpenWebUI_API_Client_With_Upload.ipynb) with a document of your own (a course reading, a README). Force the fallback path by pointing the upload at a bad endpoint, and compare the two answers you get for the same question about the document.
...
- Hands-on notebooks from this tutorial: [OpenWebUI_API_Client_With_Upload.ipynb](/files/notebooks/OpenWebUI_API_Client_With_Upload.ipynb) and [OpenWebUI_MultiAgent_Goal_Workflow.ipynb](/files/notebooks/OpenWebUI_MultiAgent_Goal_Workflow.ipynb)
