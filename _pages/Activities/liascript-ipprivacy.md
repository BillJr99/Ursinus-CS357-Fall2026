...
Today we ask two questions: who owns the material a model learned from, and who gets watched when people use it.  The *Training Data, Bias, and Explainability* activity showed what models absorb from their training distributions.  Generative models are trained on creative work and prompted with personal information.  That puts two bodies of law and ethics, intellectual property and privacy, at the center of agentic practice.  We map both *as engineers*.  The goal is not to play lawyer.  It is to recognize the decisions that carry legal and ethical weight and to design for them, with our local-first stack as a recurring answer.  The path today: the IP questions, then the privacy questions, then the regulations, then design responses you already know how to build.

---

## Directions and Group Roles

Work in your POGIL team with your rotated roles (**Manager**, **Recorder**, **Presenter**, **Reflector**).  These questions are contested, so the Reflector again guards against agreeing too early.  Nothing today is legal advice.  That disclaimer is itself a lesson in professional boundaries.  After class, respond to the reflection prompt on your own in your notebook.
...
|---|---|---|
| **Copyright** | A legal right that gives the creator of an original work exclusive control over its use and distribution for a limited time. | An author owns the text of their novel.  A model trained on that novel raises the question of whether that use was authorized. |
| **Fair Use** | A doctrine in US copyright law that permits limited use of copyrighted material without permission.  Courts weigh four factors: purpose, nature of the work, amount used, and market effect. | Courts use fair use to decide whether AI training on copyrighted books is lawful.  The answer is not yet settled. |
| **FERPA** | The Family Educational Rights and Privacy Act, a US law that restricts who may access student education records such as grades, transcripts, and advising notes. | Pasting a class roster with student names and grades into a consumer chatbot could violate FERPA. |
| **Data Minimization** | The privacy engineering principle of collecting, processing, and keeping only the data a specific task needs.  Nothing extra, nothing "just in case." | Stripping student names before running essays through an AI grading assistant is data minimization. |
| **Memorization** | When a model reproduces verbatim text from its training data in its outputs.  This creates copyright infringement risk. | Researchers have shown that repeating a prompt about a specific book excerpt can make a model reproduce full passages. |
| **Local Inference** | Running an AI model entirely on your own hardware, so no input data ever travels to a third-party server. | Using Ollama on your laptop to analyze sensitive documents keeps those documents off every external service. |
...
In this part, you learn to separate three legally distinct questions about AI and intellectual property.  News coverage routinely blurs them.  Keeping them apart helps you reason about your own projects and about the legal questions you will meet as a practitioner.

## 1.  Three Distinct Questions

Three separate questions arise where AI meets intellectual property.  Mixing them up produces confused reasoning.  Keep them apart whenever you read news coverage, policy proposals, or vendor terms of service.

Question 1, input: was training a lawful use of copyrighted work?  Training corpora include books, code, journalism, and art whose creators largely did not consent.  Lawsuits by authors, artists, and news organizations against model vendors turn largely on *fair use*, a four-factor balancing test in US law: the purpose and character of the use, the nature of the work, the amount used, and the market effect.  Courts are dividing on these questions right now.  Engineers should track the outcomes, not assume them.

Question 2, output: who owns what the model produces?  The US Copyright Office requires human authorship.  Purely machine-generated material is not copyrightable; human creative selection and arrangement can be.  For your projects, your authorship lives in your prompts, curation, code, and editorial choices, so document them.

Question 3, memorization: can the model emit its training data?  Yes, occasionally, and more often for text that repeats many times in the corpus.  Verbatim reproduction of protected work is the cleanest infringement risk.  Memorization (a model reproducing word for word the text it saw during training) is reduced by deduplication at training time and output filters at deployment.  Neither is perfect.

---

## Model 1: The Student Artist

Why this matters: every time you use a generative model for a course project, a creative assignment, or a side project, you face real intellectual property questions, even if nobody is suing you today.  Knowing where the law is unsettled helps you document your own creative contribution and avoid the practices most likely to create liability for you or your employer later.

A student trains a small image model on 400 of their own paintings.  They then also fine-tune it on 50 works by a living artist they admire, and sell outputs "in the style of" that artist.

### Critical Thinking Questions

1.  Separate the three IP questions for this scenario.  Which use is least contested, which is most contested, and why does "style" complicate the middle one?  (Style is traditionally not copyrightable, but the fine-tuning *copies* the works.)

   *Hint: Start with the simplest case: training on work you created yourself.  Then consider what changes when you add someone else's work.  Finally, think about what "copying" means: is imitating a style the same as copying a painting?*

2.  The student argues "I learned from that artist too; the model just did it faster."  Steelman this argument (state it in its strongest form), then identify the strongest disanalogy between human learning and model training.
...
3.  Given the human-authorship doctrine, what documentation would you advise the student to keep about their own contribution?

   *Hint: Think about what a copyright examiner would need to see to confirm that a human made creative choices: prompt text, selection decisions, editing steps, rejected outputs.*

---

# Part II: Privacy

In this part, you trace what happens to a prompt when you press enter, both legally and technically.  You will see why the pipelines you have already built carry different privacy obligations depending on what data you run through them.

## 2.  Where Prompts Go, and the Special Status of Student Data

Privacy is more than a personal preference.  In educational and research settings, it is a legal obligation.  Knowing which laws apply to which data, and what "sending a prompt" means for data handling, turns you from a user who hopes for the best into an engineer who designs for compliance.

A prompt is a disclosure.  Text sent to a hosted service travels through, and rests on, third-party infrastructure under that provider's terms.  Those terms may permit retention, human review, or training on your inputs.  The engineering questions are always the same: what data leaves, where it is stored, for how long, who can see it, and whether you can delete it.

Some data is regulated as well as sensitive.  In our own setting: **FERPA** (Family Educational Rights and Privacy Act) protects student education records such as grades, submissions, and advising notes.  IRB (Institutional Review Board) protocols govern human-subjects research data.  HIPAA (Health Insurance Portability and Accountability Act) governs health information.  Pasting a class roster into a consumer chatbot is bad practice, and it is plausibly a compliance violation.  GDPR (the EU's General Data Protection Regulation) and a growing patchwork of US state laws add rights of access and deletion, and limits on automated decision-making.

Agents raise the stakes mechanically.  A chatbot leaks what you paste.  An *agent with tools* can leak what it can *reach*: files, calendars, email.  The tool-permission taxonomy you built (read-only, reversible, irreversible) is privacy infrastructure.  The local stack you have run all semester is the strongest single control, because data that never leaves the machine cannot be retained by anyone else.
...
4.  For each pipeline, draw the data-flow arrow diagram: what content moves, to which process, on which machine.  Mark the first arrow, if any, that crosses your laptop's boundary.
...
> **Common Misconception:** "If I'm not storing the data, there's no privacy risk."  Many users believe that if they do not save a transcript or export a file, no data is retained.  In reality, every call to a hosted API sends your input to the provider's servers under their terms of service.  Those terms may allow retention for logging, abuse detection, or model improvement, for months or years.  "I didn't save it" and "the service didn't save it" are very different claims.
...
The case for local AI is technical as well as ethical, and the techniques have names.  Federated learning, differential privacy, and PII scrubbing each buy you something specific at a specific cost.  Knowing which is which turns a privacy preference into a design you can defend to a stakeholder.

## Why Privacy and AI Conflict

Think of AI training data like a sponge.  Once the sponge soaks up water, you cannot squeeze out one particular drop.  An LLM trained on private data absorbs personal information into billions of parameters spread across the whole model, so there is no simple "delete this person's data" button.  The analogy stops there: unlike water in a sponge, the absorbed data can come back out in recognizable form when someone prompts for it.  Differential privacy is like adding just enough noise to a survey that nobody can tell what any one person answered, while the average stays accurate.  The model learns useful patterns without memorizing individual secrets.  This tension between *learning from data* and *protecting the people the data describes* is the central problem of privacy-preserving AI.

### The Training Data Problem

Large language models are trained on huge corpora scraped from the internet and on licensed datasets.  These corpora often contain **personally identifiable information (PII)**: email addresses, phone numbers, home addresses, medical records, financial information, and personal narratives.  When the model trains on this data, it can memorize specific examples.  It does not store them as a lookup table; it stores them as statistical patterns that a targeted prompt can bring back out.

Carlini et al. (2021) showed this concretely.  By prompting GPT-2 with prefixes taken from the training corpus, they extracted:
...
This is not a bug.  It is a consequence of training on private data at scale.
...
> *Hint:* Start by asking what quasi-identifiers remain after anonymization.  A person's age, zip code, and gender together can identify most individuals in the US.  What auxiliary datasets could an attacker use to re-identify people: public voter rolls, social media, news articles?  Were rare individuals (people with unusual diagnoses or unusual names) more exposed than common ones?  Was a formal privacy model such as k-anonymity, l-diversity, or differential privacy applied, and if so, what were its parameters?

---

You now know how and why private information leaks from AI systems.  Next come the technical defenses engineers use to reduce those risks, and the specific limits of each.
...
Differential privacy gives a formal mathematical guarantee.  Take any two datasets that differ by exactly one person's record.  A differentially private mechanism produces outputs on the two datasets that are statistically indistinguishable, up to a factor controlled by the **privacy budget ε (epsilon)**.

Formally: a mechanism M satisfies ε-differential privacy if, for all adjacent datasets D and D' and all outputs S:

**Pr[M(D) ∈ S] ≤ e^ε × Pr[M(D') ∈ S]**

In plain English: if you were in the dataset and I ran the mechanism, you could not tell whether your data was included, up to a factor of e^ε.  A smaller ε means a tighter guarantee.  It also means more noise added, which reduces accuracy.
...
In traditional training, all data goes to a central server.  In federated learning:

1.  A global model is distributed to N clients (individual devices, hospitals, or partner organizations)
2.  Each client trains locally on its private data, computing gradient updates relative to the current global model
3.  Only the gradient updates (mathematical vectors describing how to improve the model) go to an aggregation server; raw data never leaves the client
4.  The server aggregates the updates (typically by weighted averaging) to produce an improved global model
5.  The improved global model goes back to the clients, and the process repeats
...
**Question 5.**  In 2019, Zhu et al. showed that a malicious or curious aggregation server in a federated learning setup could reconstruct training images from gradient updates, a **gradient inversion attack**.  How does this change your assessment of federated learning as a privacy solution for medical imaging?  What defenses exist, and what do they cost?

> *Hint:* The three main defenses are: (1) Adding DP noise to each client's gradient updates before they are sent to the server.  This works, but it reduces model accuracy and requires tuning ε for the gradient space, not the output space.  (2) Gradient compression, dropping or quantizing small gradient values.  This reduces the information available for reconstruction but also slows convergence.  (3) Secure aggregation using cryptographic protocols such as homomorphic encryption or secure multiparty computation.  The server receives only the sum of all gradients, never an individual update, but this requires significant computational overhead.  What does each defense cost in model quality, infrastructure complexity, or training time?
...
> *Hint:* This sentence contains: (1) implied health information (the customer had surgery), which is sensitive medical data; (2) the name of a healthcare facility, which narrows location and implies a relationship with that institution; (3) an approximate date, which combined with other data could help identify the individual.  An NER model would likely catch "Phoenixville Hospital" as an ORG entity and "last March" as a DATE.  An LLM-based scrubber is the only approach likely to recognize that "my surgery" is itself a health disclosure that should be redacted.  What does it even mean to "remove PII" when the information is implied by context rather than stated as a named field?
...
> **Why this answer?**  Federated learning keeps raw patient data on-premises, which is the core requirement when data cannot leave the institution.  Anonymization is insufficient because clinical notes often contain re-identifiable combinations of rare diagnoses, facility names, and dates; removing names alone does not prevent re-identification.  Passing Protected Health Information (PHI) through a vendor's API in system prompts almost certainly violates the HIPAA Business Associate Agreement requirements that govern how vendors may use that data.  Inference-time DP adds noise to outputs but does nothing about training data exposure; the model was already trained on private data before any DP was applied.

---

You now have the theoretical defenses.  Next you apply them in the day-to-day operation of a real AI agent system, where PII enters through user inputs, can echo back in outputs, and must be managed through a layered logging and deletion policy.

## Practical PII Handling for Agents

### The Three-Layer Defense

When an AI agent processes user inputs and produces outputs, PII can enter and leak at several points.  A well-defended system checks at every layer.

**Layer 1, input scrubbing**: Before the user's message goes to the LLM, detect and redact PII.  Replace each identified item with a typed placeholder: `[PERSON_NAME]`, `[PHONE_NUMBER]`, `[SSN]`.  Keep a session-scoped mapping table if the agent needs to dereference a placeholder later (for example, to address the user by name in a response without the LLM itself storing the name).

**Layer 2, output scrubbing**: After the LLM generates a response, scan it for possible memorized PII (email addresses, phone numbers, names from training data) before returning it to the user.  Log flagged instances for human review, and consider blocking the response if high-confidence PII is detected.

**Layer 3, logging policy**: State explicitly what the agent system keeps and for how long:
- What conversation data is stored, and where?  (User device, company servers, third-party logging provider)
- For how long is it kept?  (Session only, 30 days, indefinitely)
...
GDPR Article 17 grants individuals the right to erasure: the right to have their personal data deleted.  For most databases this is straightforward: delete the row and cascade the deletion to backup tables.  For AI models, it is deeply problematic:

- If a person's data was included in training, their information is encoded into billions of parameters spread across the entire model, not stored as a retrievable record
...
- No current approach is both computationally efficient and formally verifiable; the model cannot certify that the data's influence has been fully removed

### Relevant Regulations

- **GDPR** (European Union, effective 2018): Mandates data minimization, purpose limitation, the right to erasure, data protection by design and by default, and breach notification within 72 hours of discovery
...
> *Hint:* A reasonable protocol has these steps: (1) Detect the SSN *before* it reaches the LLM using a pre-processing layer, so the LLM never sees the raw SSN.  (2) Redact the SSN in any stored log immediately and irreversibly.  (3) Complete the actual task (the order status lookup) using only the account number, which is the right identifier for this task.  (4) Decide what to tell the user: do you warn them that you detected a sensitive number and did not store it?  Do you recommend they contact support through a more secure channel?  Be specific about the wording of the response.

---

**Question 8.**  A user requests deletion of their data under GDPR.  Your company confirms it has deleted their account record from the production database, the conversation logs, and all backups.  But a fine-tuned model was trained on their support conversations six months ago and is serving production traffic today.  Is the GDPR obligation satisfied?  What practical options exist for handling personal data embedded in model weights?

> *Hint:* The legal question is whether personal data "encoded in model weights" counts as personal data in the GDPR sense.  The model (usually) cannot reproduce the person's exact conversations verbatim, but their information influenced the weights.  The UK Information Commissioner's Office (ICO) has begun addressing this question.  Practical options include: (1) approximate machine unlearning, fine-tuning the model on a dataset that excludes the person's data; (2) full retraining without the person's data, which is expensive; (3) documenting to the user that weight-embedded data cannot be deleted with current technology and describing what was deleted; or (4) designing from the start not to include personal data in fine-tuning datasets.  Which of these is most defensible legally?  Which is most achievable in practice?
...
You now know both IP and privacy as legal frameworks.  This part puts them to work: reading real terms of service, probing memorization in your local model, and writing the compliance memo your project will need.

## 3.  Exercises

1.  *Terms-of-service safari.*

   *What to do:* Each teammate reads the data-use terms of one AI service they actually use and extracts three things: the retention period, the training-on-inputs policy, and the deletion rights.  The Recorder builds a comparison table.  The Presenter reports the most surprising finding.

   *Starter hint:* Search the service's terms of service or privacy policy for the words "training," "retain," "delete," and "opt out."  Many services put opt-out options in account settings rather than in the terms themselves, so check both.

   *You've succeeded when:* Your team has a table comparing at least three services on the same three dimensions, and the Presenter can explain in plain language what happens to a prompt after you press enter on each service.

2.  *Memorization probe.*

   *What to do:* Try (politely) to get your local model to produce verbatim famous text, using increasingly specific prompts.  Report the level of specificity, if any, at which reproduction begins, and connect the result to the repeated-text mechanism above.

   *Starter hint:* Start vague ("write a sentence about a boy wizard") and add specificity step by step ("write the opening line of Harry Potter") until you see verbatim text or confirm that the model will not produce it.  Note which prompt phrasings trigger which behaviors.

   *You've succeeded when:* You can describe the threshold at which reproduction occurred (or did not), state a hypothesis about why, and connect your finding to training-data memorization through repetition frequency.
...
3.  *Local-first justification memo.*
   ...
   *You've succeeded when:* A campus IT officer who has never seen your project could read the memo, understand the data flows, and decide whether to approve the system for use with real students.

4.  *Case watch.*

   *What to do:* Find one active AI copyright or privacy case or regulatory action (search the news), and summarize in two sentences the question it will settle.  We pool these in the *Governance, Policy, and the Cost of Inference* activity as our survey of active cases.
...
*Personal:* You have free, private, capable AI on your own laptop, an option most users do not know exists.  When you use a local model for personal tasks that involve sensitive information (health questions, relationship problems, financial concerns), does knowing how to keep data local change how you feel about using AI at all?

*Technical:* You can now build both local and cloud-connected AI pipelines.  What technical obligations do you have when you build systems that other people will use, including people who do not understand where their data goes?

*Societal:* Most people who use AI assistants cannot run local models, cannot read terms of service, and cannot see how their inputs are used.  Does the gap between technically informed and technically naive users create a fairness problem?  Who should close it, and how?
...
Optional, and separate from the parts above.  Today's session argued the property and privacy case.  Underneath it sits a harder question the law is not equipped to settle: what authorship means when the work is generated, and whether originality survives the process.  Bring this one to your Reading Group if it grabs you.
...
| **Transformational Creativity** | Changing the rules of a creative domain itself, rather than producing something new within the existing rules: rewriting what the rules allow. | Cubism did not just paint differently; it redefined what painting was allowed to represent |
...
| **Fair Use** | A US copyright doctrine that permits certain uses of copyrighted material without permission, based on four factors including transformativeness and market impact.  Currently at the center of most AI copyright litigation. | Whether training a generative model on millions of copyrighted images is fair use is unresolved in US courts as of 2025 |
| **Moral Rights** | Rights beyond copyright that protect a creator's connection to their work, including the right of attribution and the right to object to distortions.  Stronger in Europe than in the US. | A European artist has the right to be credited as the creator of their work even after selling the copyright; an AI-mimicked style may violate the spirit of this even if no specific work is copied |
...
Ask someone whether an AI can be creative and you will get a confident "yes" or a confident "no."  The two confident people are probably thinking about different things when they say "creative."  Cognitive scientist Margaret Boden gave the field a more useful tool: not a yes/no question about creativity, but a taxonomy of three *types* of creativity that do different things and can be evaluated separately.  AI systems clearly do some of these types well; others remain debated.  By the end of this activity, you should be able to say something more precise than "AI is/isn't creative."  You should be able to say *which type* of creativity is at issue in any specific claim.

#### Boden's Three Types of Creativity

**Combinatorial creativity:** Producing novel and surprising combinations of familiar elements.  Most everyday creativity is combinatorial: a poet connects two concepts that are rarely paired, a chef combines ingredients from different cuisines, a programmer applies a sorting algorithm to a problem it was not designed for.  The space of possible combinations is vast but fixed.  Creativity here means moving through it in a surprising and useful direction.

**Exploratory creativity:** Systematically exploring the edges of an existing conceptual space, pushing a style, genre, or tradition to its limits while still following its structural rules.  A jazz musician improvising within bebop conventions, or a mathematician working out the consequences of a given axiom system, is practicing exploratory creativity.  The rules are given; the creativity is in how far the exploration goes.

**Transformational creativity:** Changing the rules of the conceptual space itself, so that previously impossible or inconceivable ideas become possible.  Cubism did not just paint differently; it redefined what painting was allowed to represent.  This type is rarer and more disruptive than the other two.
...
> *Hint:* Random selection produces novelty in a trivial sense: the sentence "purple philosophy sleeps honestly" is novel because it has probably never been written before.  But most people would not call it creative.  What is missing?  Consider: intentionality (did something choose this because it expected a particular effect?), coherence (does it mean something?), appropriateness (is it surprising in a way that resonates with the audience?), and purpose (was it produced to achieve something?).  Does the process matter?  If a human and a random word-picker both produce the same novel sentence, are they equally creative?  Or does searching purposefully through a space of possibilities matter independently of the output?

---

**Question 2.**  When a human audience judges something as creative, what role do their expectations and cultural context play in that judgment?  Could the same output count as creative in one context and completely uncreative in another?  Give a concrete example.

> *Hint:* Consider Marcel Duchamp's "Fountain" (1917), a urinal submitted to an art exhibition.  In the avant-garde art world of 1917, it was considered radically creative because it challenged the definition of art itself.  The same object in a hardware store in 1917 was not creative at all; it was plumbing.  The object did not change; the context and the audience's expectations did.  Now consider AI-generated poetry being judged as creative.  Does it matter whether the audience knows an AI wrote it?  Would the same poem be judged more or less creative if the audience knew the process?
...
> *Hint:* Consider Einstein's special relativity (1905).  It transformed physics by changing what time and space were allowed to mean, a transformational move.  But it built on Lorentz transformations that already existed, on Maxwell's equations that had already been derived, and on a decades-old exploratory tradition of thought experiments.  Was the transformation in the specific insight Einstein had, or in the combination of existing elements into a new framework?  Can transformation happen *without* prior combination and exploration, or does transformational creativity always stand on the shoulders of combinatorial and exploratory work?

---

With Boden's framework in hand for evaluating creative claims, you can now examine the legal and ethical context in which AI creativity is being deployed and contested.

### Copyright, Attribution, and the Artist's Dilemma

Generative AI has created a legal and ethical crisis in creative fields.  Public debate often blends three separate issues.  Keep them distinct, because they have different legal frameworks, different affected parties, and different possible remedies.
...
AI systems can generate output explicitly "in the style of" a named living artist.  Under current US copyright law, **style itself is generally not copyrightable**; only specific original expression is.  You cannot copyright "impressionism" or "Hemingway's prose style" as such.  But many artists feel their livelihood is directly undermined when anyone with a text prompt can replicate their distinctive style at scale, without payment, permission, or attribution.

Moral rights (recognized more broadly in Europe than in the US) include the right of attribution (to be identified as the creator of your work) and the right of integrity (to object to distortions or modifications).  Some argue that AI style mimicry at scale violates the spirit of moral rights even where it does not technically violate copyright law.
...
> **Common Misconception:** Many students assume that "AI-generated content cannot be copyrighted" or "AI-generated content is always in the public domain."  The legal reality is more complex.  In the US, the Copyright Office has stated that purely AI-generated content with no human creative input is not copyrightable.  But work where a human makes specific creative choices (selecting, arranging, editing, and directing AI outputs) may be protectable to the extent of the human contribution.  The exact threshold of human involvement required for protection is being litigated and worked out through Copyright Office guidance as of 2025.

#### Questions to Work Through

**Question 4.**  An AI generates an image using a model trained on 5 million images, and no single training image can be identified as a direct visual template for the output.  Has copyright infringement occurred?  What legal test would you apply, and what additional facts would you need to apply it?

> *Hint:* US copyright infringement requires showing: (1) the plaintiff owned a valid copyright in the original work; (2) the defendant copied protected expression from that work.  The second element is usually proven by showing access (the defendant saw the work) and substantial similarity (the output is too similar to be coincidental).  At mass scale, access is trivially established; a model trained on the web clearly had access.  The harder question is substantial similarity: if no specific work can be identified as the source of any specific output, is there infringement?  Some legal scholars argue that the *training process itself* is the infringement, not the specific output.  Others argue that only output-level similarity matters.  What additional facts would change your analysis?

---

**Question 5.**  A musician has spent 20 years developing a distinctive vocal style and has posted hundreds of recordings publicly online.  An AI company trains a voice-cloning model on those recordings without asking permission.  The company generates no tracks identical to the musician's recordings.  Why might the musician still feel (and arguably be) harmed?  What interests beyond copyright are at stake?

> *Hint:* The musician's interests go beyond copyright: (1) **Economic harm**: if AI can generate unlimited music "in their voice" at zero marginal cost, the market for their recordings may collapse even if no specific recording is infringed; (2) **Right of publicity**: some jurisdictions protect the commercial use of a person's voice, name, and likeness independently of copyright; (3) **Autonomy and consent**: the musician may have views about what messages, genres, or contexts their voice should be associated with, and voice cloning removes their control over this; (4) **Dignity**: having one's voice used to generate content one would find offensive or objectionable.  Which of these interests, if any, are addressed by existing law?  Which require new law?

---

**Question 6.**  Propose a specific attribution policy for AI-assisted creative work that you consider fair to all parties.  Your policy must address: (a) what creators of training data are owed, (b) what AI system developers may do without additional compensation, and (c) what users who produce AI-assisted work must disclose to audiences and consumers.  Defend each of your three choices explicitly.

> *Hint:* This is a real design problem with tradeoffs at every step.  Creators of training data could be owed nothing (current US law leans this way), a one-time license payment, or an ongoing royalty every time their work influenced a generation.  AI developers might be allowed to train on publicly posted work without permission (the transformative use argument) or might be required to use only licensed or public-domain works.  Users might be required to disclose AI assistance whenever more than X% of the final work was AI-generated, but how do you measure that percentage?  Choose specific positions and defend them.  Identify the hardest tradeoff in your policy.
...
> **Why this answer?**  As of 2025, no US court has issued a final ruling on whether training generative AI models on copyrighted works is infringement.  The cases are ongoing.  Style (as distinct from specific original expression) has historically not been protectable under US copyright law: you cannot copyright "impressionism" or "a distinctive color palette."  But whether training on copyrighted work is fair use remains open, and the answer may depend on the scale of copying, the commercial purpose, and the market impact on the original creator.  "Publicly posted" does not mean "in the public domain."  Copyright attaches automatically to original creative work at the moment of creation, not upon registration or upon certain types of publication.

---

The legal debates around training data and style mimicry set up the last topic: the full range of ways humans and AI systems can work together creatively.  This is where you have the most direct agency as a builder.

### Human-AI Creative Collaboration

Generative AI does more than replace human creativity.  Increasingly it collaborates with it, along a wide spectrum.  Where a particular human-AI creative relationship sits on that spectrum decides questions of authorship, attribution, copyright, and how to judge the work's quality.
...
**"Now and Then" (The Beatles, 2023).**  Paul McCartney used AI audio separation technology (developed by Peter Jackson's team during the "Get Back" documentary) to isolate John Lennon's vocal track from a low-quality 1970s home demo tape.  All four original Beatles contributed to the finished song, including archived guitar parts from George Harrison.  Questions of authorship (is a 50-year-old reconstructed performance an authored contribution?), the resurrection of a deceased artist's voice, and consent from estates all arose at once.  Was this restoration or creation?  Does the answer change if the voice was 95% AI-reconstructed rather than 30%?
...
As AI pushes the marginal cost of generating creative content toward zero, the supply of generated content explodes.  Economic theory suggests that an oversupply of algorithmically generated content will drive down the value of average-quality creative work, while raising the premium on work that is novel, contextually embedded, or deeply rooted in specific human experience.  Authentic human perspective and embodied experience may become *more* economically valuable, not less, as AI-generated content floods the market.  This is not guaranteed.  It depends on whether consumers can tell AI-generated work from human-generated work, and whether they care to.

#### Questions to Work Through

**Question 7.**  In the "Now and Then" case, is the song a human-created song or an AI-assisted song?  Does your answer depend on how you define "the work": the final mixed recording, the original demo, the performance decisions, the songwriting?  Does it depend on who owns the masters, who receives royalties, or what the listening audience believes about how it was made?  Is there a single objectively correct answer, or does the question itself reveal the limits of our existing categories?

> *Hint:* The song's elements are: Lennon's 1970s vocal performance (clearly human-authored, though AI-processed to extract it); McCartney's new bass parts (human-authored in 2023); Harrison's archived guitar parts (human-authored before his death); Starr's new drums (human-authored in 2023); and the AI audio separation that made the restoration possible (tool or co-creator?).  Different definitions of "the work" (the songwriting, the performances, the production) give different answers about AI's role.  Compare this to film: a director does not operate the camera, light the scene, or edit the footage, yet we attribute authorship to the director without hesitation.  What makes directorial creative choices authorship while AI prompt choices might not be?
...
**Question 9.**  Name one creative domain where AI collaboration is clearly beneficial: the benefits outweigh the harms, and the risks are manageable enough that you would not hesitate to recommend it.  Explain your reasoning.  Then name one domain where you are most uncertain about the net effect, and describe specifically what evidence would help you decide.

> *Hint:* For the clearly beneficial case, look for domains where the creative task has a clear, measurable outcome (the drug works or it does not; the code runs or it does not); human expertise stays in the loop for all deployment decisions; the AI speeds up exploration without replacing human judgment; and the people affected by the outcome have consented to AI involvement.  For the uncertain case, look for domains where the line between augmentation and replacement is unclear; the audiences who consume the output cannot tell whether AI was involved; the economic effects on human practitioners are severe and concentrated; or the creative choices encode values that matter deeply to communities who were not consulted.  What specific data would turn your uncertainty into a clearer assessment?
...
*What to do:* Use an AI tool to generate a creative artifact, then revise it substantially until you feel real ownership of the result.  Then reflect carefully on the process.

*Starter hint:* Try this prompt for a starting point: "Write a short poem (8-12 lines) about the experience of debugging code at 2 AM, using extended metaphors from nature: storms, erosion, something slow and inevitable.  Do not rhyme."  Take the poem the AI generates and revise it: change word choices that do not feel right, add an image from your own experience, cut lines that feel generic, shift the ending to land differently.  Keep revising until you would be willing to put your name on it.

*You've succeeded when:* You have documented (a) the exact AI-generated original, (b) your final revised version, (c) a line-by-line or section-by-section account of what you changed and why, and (d) a one-paragraph reflection on the point in the process (if there was one) where the work shifted from "the AI's poem I'm editing" to "my poem that started from an AI draft."  If that shift never happened, explain why not.
...
*What to do:* Propose a licensing framework for AI-generated creative work that balances the interests of four stakeholder groups.  Write your framework as a structured policy document.

*Starter hint:* Your four stakeholder groups are: (1) Creators whose work was used as training data: what are they owed, and is it practical to identify and compensate them individually?  (2) AI system developers: what uses are permitted without additional licensing, and what requires explicit permission?  (3) Users who prompt AI to produce outputs: what rights do they have to the outputs they direct?  (4) Consumers and audiences of the final creative work: what do they have a right to know about how it was made?  For each group, first identify what they most want, then identify the constraint from another group that makes getting everything impossible.  Your hardest tradeoff is probably between groups 1 and 2.
...
**Personal:** Have you used AI for a creative task: writing, coding, design, music?  If so, how did it feel compared to working without AI?  Did the output feel like yours?  If you have not used AI creatively, describe what you imagine the experience would feel like, and what would determine whether the result felt authentic.

**Technical:** Technology has always changed what creativity means.  Photography made realistic portraiture available to everyone and freed painters to become Impressionists.  The printing press created an author economy that had not existed before.  Synthesizers gave solo musicians orchestral textures without an orchestra.  Is AI different in kind from these earlier disruptions, or different only in degree?  Earlier technologies augmented human creative capacity.  Does AI do the same, or does it replicate it?  What specifically would make AI different in kind rather than in degree?
