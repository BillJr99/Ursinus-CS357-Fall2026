...
| Reasoning trace | The long intermediate stream a reasoning model emits before its answer. Generated by the same mechanism as the answer, so it is evidence *about* the judge's process, not a log of it, and never a substitute for a quotation from the source | The trace that argues confidently for level 3 while quoting a sentence the essay never contained |
...
| 40-65 | Part III, audit the judge: find a bias, then measure it, including what changes when the judge is a reasoning model |
...
Sooner or later someone on your team will propose using a **reasoning model** as the judge, on the theory that a model which deliberates before answering will grade more carefully.  That instinct is half right, and the half that is wrong is dangerous enough to be worth naming before you build the Rubric Pipeline Lab on top of it.

**What it genuinely buys.**  Multi-criterion rubrics are exactly the shape of problem extra dependent steps help with: hold four criteria in mind, check the essay against each, keep the levels straight, and do not let a strong showing on criterion 1 bleed into criterion 3.  A reasoning model is measurably steadier at that bookkeeping than a single forward pass, and steadiness is most of what you want from a judge.

**What it does not buy, and what you must not do with it.**  The visible reasoning stream is generated text, produced by the same next-token mechanism as the verdict.  It reads like the judge's justification.  It is not, necessarily: research on chain-of-thought faithfulness finds cases where a model's stated reasoning does not match what actually drove its answer.

> **Never paste a reasoning trace into a rubric as evidence.**  Your judge prompt demands quoted evidence *from the essay* for every score, and that requirement is doing real work: a quote is checkable against the source, and a trace is not.  A trace saying "the essay lacks a clear thesis" is the judge asserting the score a second time in longer form, not supporting it.  Swapping checkable evidence for persuasive narration is a downgrade that looks like an upgrade, and it is the specific failure this section exists to prevent.

There is a second-order effect worth watching for.  A long, fluent, confident trace is more persuasive to *you*, the human auditing the judge.  The pathologies you catalogued in Part I do not disappear when the judge thinks longer; they arrive better argued.  Verbosity bias in particular gets worse, since the judge now produces verbosity of its own that you have to read past.

### One More Audit Step

Add this to the exercises below, whichever judge you end up using.  It costs one extra run and settles the question empirically for your rubric:

**Score the same essays twice, once with the reasoning trace shown to you and once with it hidden**, and compare your human agreement numbers across the two conditions.  If your agreement with the judge goes *up* when you can see its reasoning, ask the uncomfortable question: did the trace help you evaluate the score, or did it persuade you to accept it?  Those look identical from the inside, and only the numbers can tell them apart.

#### Critical Thinking Questions

7.  Your judge is a reasoning model.  For one essay it scores criterion 2 at level 3, and its trace argues carefully for level 3 while quoting a sentence that does not appear anywhere in the essay.  Name what went wrong, and say which of your existing checks would have caught it and which would not.
