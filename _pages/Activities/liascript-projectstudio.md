...
Project roles (rotated by sprint) are in effect today: **Coordinator**, **Builder(s)**, **Evaluator**, **Scribe**.  The Scribe keeps today's living document: the stand-up notes, every piece of gallery feedback written down verbatim, and the triaged backlog you leave with.
...
|------|--------------------------|--------------------------|
| **Stand-up** | A brief, structured team status check.  Each person answers the same fixed questions in two minutes or less.  Its purpose is to surface blockers and share numbers, not to impress anyone. | "The evaluation harness reports 73% tool-call accuracy on our test set.  The riskiest unfinished piece is the citation pane." |
| **Gallery walk** | A peer review format where teams rotate between each other's live demos and leave structured written feedback.  It is the academic version of a design critique or code review. | Walkers leave one index card per station with exactly three fields: Strength, Question, Risk |
| **Triage** | Sorting all feedback into exactly three buckets by severity and scope.  It is the discipline of deciding what must be fixed, what must be disclosed, and what must wait. | Feedback: "The retrieval returns empty results for short queries" -> Fix before demo |
| **Release-readiness checklist** | A concrete, signed-off list of verifiable conditions that must be true before a system is shown publicly.  Borrowed from the definition-of-done practice in software engineering. | Six items, each answerable yes or no, signed off by the Evaluator |
| **Known failure case** | A defect or limitation the team has identified, documented, and can demonstrate on demand.  The opposite is a hidden failure that someone else discovers during the demo. | "When the knowledge base has fewer than three documents, the system returns a hallucinated citation" |
| **Reproducibility** | The property that another person following your written setup steps, on a fresh machine, gets the same results you do.  It requires fixed seeds, pinned model versions, and documented steps. | A teammate who did not write the setup document runs the system from scratch using only the README |
...
Each team answers exactly four questions, in two minutes at the board: What works end-to-end today?  What is the riskiest unfinished piece?  What did your evaluation harness report this week (a number, not an adjective)?  What do you need from the instructor or another team?  A stand-up is status synchronization, not a performance.  The discipline is *saying the number*.

### Critical Thinking Questions

**Question 1.**  Your team's evaluation harness reports a metric (for example, 68% retrieval precision, or 3.2/5 average response quality).  Before the stand-up, your Coordinator says "let's say it's around 70%; sounds better."  What is wrong with this approach, and what is the actual function of reporting the exact number?

[[___ Your answer here ___]]

*Hint:* Who else in the room might be able to help your team improve that metric if they know the exact number?  What decisions (about where to spend the final sprint) depend on the accurate number?  Also consider: if the number is 68% and the threshold for a good demo is 75%, rounding to "around 70%" hides the gap that needs closing.

**Question 2.**  "The riskiest unfinished piece" requires the team to have already thought about failure modes.  List three categories of risk that are common in AI systems but rarely mentioned in student project stand-ups, and explain why each one is hard to surface without asking for it explicitly.

[[___ Your answer here ___]]

*Hint:* Think about risks that stay invisible until something goes wrong: latency (the system is slow on long documents), edge cases (what happens when the user asks a question the system was not designed for), and dependency risks (what if the external API the system relies on changes its pricing or rate limits before demo day?).

---

*You have put your team's blockers and metrics on the table in the stand-up.  Now leave your station to evaluate other teams' work critically, and host visitors at your own.  The gallery walk gives everyone a structured role and a specific three-field card to fill out, so no feedback is wasted on vague praise.*

## Model 2: Gallery Walk Protocol

The gallery walk is structured so that you get useful feedback rather than polite feedback.  The card format forces specificity.  "Good job on the UI" does not help anyone; "the citation pane made the RAG answer checkable in one glance" is a named strength that can be preserved and built on.  In the same way, "the demo looked fine" is not a risk assessment; "the system fails silently when retrieval returns nothing" is a named risk with a clear fix path.

### 2.  Gallery Walk Protocol (40 minutes)

Stations: each team's system runs live, with its one-page architecture diagram and its current evaluation table beside it.  Half of each team hosts and half walks; swap at the midpoint.
...
- **Question**: one real question the demo raised, ideally about a seam, a failure mode, or governance ("what happens when retrieval returns nothing?").
- **Risk**: the one thing most likely to fail on demo day, stated kindly and concretely.

Hosts demonstrate honestly.  Every station must show at least one *known failure case*: a defect or limitation your team has already identified, documented, and can reproduce on demand, the opposite of a surprise discovered mid-demo.  A demo that hides its failure modes is rehearsing a deception, and your governance documents say otherwise.
...
*Hint:* Questions that expose unconsidered failure modes often come from visitors who tried to use your system in a way you did not design for.  Did any walker paste a very long input, ask about a topic outside your training data, or ask what happens when the system is wrong?  The question that makes you say "we actually don't know" is the most valuable one.
...
*Hint:* Explainability affordances are the parts of a system that help users understand *why* it said what it said.  A trace shows the reasoning steps.  A citation shows which source the answer came from.  A confidence display shows how certain the model is.  Be specific: if you added the affordance you admired to your own system, what would need to change in your architecture to support it?

---

> **Common Misconception:** Students sometimes treat the gallery walk card as a formality and write vague feedback ("nice work," "interesting approach") to avoid seeming critical.  This defeats the purpose of the exercise.  Vague positive feedback gives the receiving team nothing to act on.  The most useful feedback you can give is a concrete risk or a real question: something the team can investigate and either fix or explicitly disclose.  Kindness and specificity are not opposites.
...
For each piece of feedback your team received, the Coordinator reads it out, the team agrees on the bucket, and the Scribe records:
...
*Hint:* The hardest items to triage are the ones where the team disagrees about whether something is "core" or "peripheral."  Here is the test: if this failure happens during the demo and you have not disclosed it, does it undermine the audience's understanding of what the system does?  If yes, it is Disclose at minimum, and Fix if there is time.

---

*With your backlog triaged and your disclosures drafted, the final step is a formal readiness check.  The Evaluator's sign-off is valid only if each "Yes" comes with evidence (a time, a link, a run result) and not confidence alone.*

## 4.  Release Readiness Checklist

Before demo day, every team verifies each item and the Evaluator signs off on it.  "Yes" requires evidence (a number, a transcript, a test run), not belief alone.
...
The Evaluator signs off only when all six items are Yes with evidence.  A partial checklist is not done.
...
*What to do:* Each team member writes their own answer to all four stand-up questions.  Then compare answers as a team.  Where do your answers disagree about the riskiest item or the metric?  Disagreements are informative.
...
*You've succeeded when:* All four questions have written answers with specific numbers or named artifacts, and no adjective stands in for a measurement.
...
*You've succeeded when:* Each disclosure names the specific limitation, explains why it exists, and gives the audience enough context to adjust their evaluation.
...
**Personal level:** Compare the feedback you received today with the feedback your own critique agents give in your system.  Which was easier to act on: human peer feedback or automated evaluation?  What does that comparison teach you about what human judgment adds that automated metrics cannot capture?
...
**Societal level:** The gallery walk requires showing a known failure case at every station.  In commercial AI deployments, failure cases are rarely demonstrated publicly.  What incentives lead companies to hide failure modes, and what would it take (regulation, liability, cultural norms) to make candid failure disclosure the default rather than the exception?
