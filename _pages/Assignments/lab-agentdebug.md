---
layout: assignment
permalink: /Assignments/AgentDebug
title: "CS357: Foundations of Artificial Intelligence - Lab: Debugging a Broken Agent"
...
info:
  ...
  rubric:
    - weight: 40
      ...
      preemerging: Fewer than two bugs correctly identified and fixed
      beginning: Two bugs fixed with correct code but no explanation
      progressing: Four bugs fixed with explanations of symptom, cause, and fix
      proficient: All five bugs fixed; each fix is accompanied by (1) the observable symptom that revealed it, (2) the root cause, and (3) what the fix does mechanistically; the fixed agent passes all five test cases
    - weight: 25
      description: Structured Logging
      preemerging: No logging added
      beginning: Print statements added at key points
      progressing: Structured logging with step counter and timestamps added
      proficient: Logging records step number, tool name (if called), tool arguments, tool result, response length, and elapsed time per step; log output written to a file AND displayed on console with appropriate severity levels (INFO for normal flow, WARNING for unexpected behavior, ERROR for failures)
    - weight: 20
      description: Test Suite
      preemerging: No tests written
      beginning: One test case
      progressing: Three test cases covering normal and failure paths
      proficient: Five test cases covering; successful fact retrieval, arithmetic chain (two tool calls), empty query input, unknown topic (model should abstain rather than confabulate), and step budget enforcement (agent must stop within max_steps); each test prints PASS/FAIL with the actual output
    - weight: 15
      description: Writeup and Reflection
      preemerging: No writeup
      beginning: Writeup lists bugs found
      progressing: Writeup explains why each bug was hard to find
      proficient: Writeup explains the diagnostic process for each bug (not just the fix), identifies which bug was hardest to find and why, and proposes one structural change to the agent architecture that would prevent a whole class of bugs
  readings:
