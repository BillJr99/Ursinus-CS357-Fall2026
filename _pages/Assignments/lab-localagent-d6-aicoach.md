...
At minimum, support **one** provider end to end; structure the function so a second provider is a small addition. A recommended, keyless starting point is a **local OpenAI-compatible server** (Ollama or Open WebUI), exactly as in the [REST activity]({{ site.activity_url }}liascript-restllmapi.md):
