...
The network outputs raw **logits** <span>\\(z\\)</span>, which can be any real numbers. To interpret them as probabilities over classes (that add up to `100%` probability, or `1.0`), we apply the softmax function: $\hat{y}_i = \frac{e^{z_i}}{\sum_{j=1}^C e^{z_j}}$
