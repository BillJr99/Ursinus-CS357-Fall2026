...
# Quick Reference (formulas we used)

- **Linear forward:** `out = X @ W.T + b`  
- **Linear backward:** `dW = dout.T @ X`, `db = sum(dout, axis=0)`, `dx = dout @ W`  
- **ReLU:** `relu(x) = max(0, x)`; `relu_backward(z, dout): dout where z>0 else 0`  
- **MSE gradient wrt ŷ (batch):** `(2.0/bs)*(ŷ - y)` if your MSE is defined **without** the ½.  
- **Softmax + CE backward (given):** `dlogits = (probs - Y_true)/batch`