...
Go in order here.  Every section leans on the previous one, and you should run each code block when you reach it.
...
**Wrangler** is the command-line interface to all of it: it scaffolds projects, runs them locally, deploys them, and manages secrets, which makes it the `docker` of this tutorial.  The framing for our course: the local stack is where private things live; Cloudflare is where *shareable, non-sensitive* things go, and deciding which is which is a governance exercise you have already trained for.
...
- wrangler.toml is committed to the repository, so the key becomes part of the project's public record; secrets must go through wrangler secret put, which stores them server-side only
...
Before ruling, name the deciding principle for each case out loud; if you are working with a partner, make sure you agree on it.
...
*Hint:* "We don't store it" is a claim about *your* system, but what about the third-party LLM provider the Worker forwards to?  And the essay author, the student: did they consent to having their work sent to a cloud service for automated scoring?  These are the two questions.
...
- A repository *environment secret* attached to a protected `production` environment, so it is encrypted, masked in logs, and only usable after the environment's approval rule passes
...
- Restrict the trigger to `main` (plus `paths` filters) and move the full deploy behind `workflow_dispatch`, so it runs on demand rather than on every commit
...
Deployment used to be the moat that separated people with servers from everyone else, and this tutorial reduced it to a login and a one-word command.
...
In the Project Studio, you will take everything built and published this semester and prepare it for the final gallery walk, including rehearsing failure cases and triaging feedback into your final sprint.
