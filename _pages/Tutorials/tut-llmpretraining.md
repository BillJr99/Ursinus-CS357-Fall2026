...
Do these in order.  Each section builds on the last, and you'll get much more out of the code blocks by running them.
...
- The most frequently occurring adjacent pair of tokens in the current tokenized corpus, merging them into a single new token
...
## Understanding the Training Signal
...
- Causal Language Modeling, predicting the next token given all previous tokens, with a causal attention mask preventing access to future positions
...
## Scaling and Quantization Decision Table
