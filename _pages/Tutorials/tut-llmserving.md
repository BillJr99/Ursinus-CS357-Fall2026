...
- TTFT: the time to first token is too high, causing the perceived "blank" period before text begins
...
- It sits idle until all 7 remaining requests finish their 400 tokens, then the entire batch is cleared
...
- Internal fragmentation: the waste is reserved-but-unfilled space inside the request's own block
...
- A request's logical block addresses to the physical block addresses in VRAM where the KV data actually sits
...
- Prefix caching: shared prefixes are hashed and stored once, then pointed to
...
**Personal**: The optimization metrics in this tutorial (TTFT, TPOT, throughput, cost) are all engineering abstractions.  But they map directly to user experience: the frustration of a blank screen, the satisfaction of text that flows as fast as you read.  Which metric do you find you care about most as a *user* of AI systems, and did this tutorial change how you think about why that experience feels the way it does?
