...
- Embedding lookup, transformer layers, and output layer
...
- Its pre-activation ($-0.5$) was negative and ReLU clips negative values to zero
...
## Weight and Activation Heatmaps
...
11.  Connect all four Parts in two sentences: where in the Part I pipeline does the Part II arithmetic happen, and where in the pipeline do the Part IV vectors live?
...
- An embedding table is a layer of learned weights, so an embedding is a learned representation, the same kind of re-description a hidden layer performs
