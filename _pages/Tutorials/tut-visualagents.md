...
## Build 2 - RAG over Your RAG Knowledge Base Lab Corpus
...
## Build 3 - Export and Reenter Code
...
- Both express the same underlying patterns; visual excels at communication and rapid wiring, code at version control, testing, and arbitrary logic
...
Open `http://localhost:7860` and keep notes on what each component represents and how it maps to the Python code from previous labs.
...
- The system message in the `messages` list (the dict with `"role": "system"`)
...
In the *Evaluating Agents With a Rubric: The Judge Pipeline Workshop* activity, we measure agent output quality at scale, which requires recruiting a model to act as the judge of other models' outputs.
